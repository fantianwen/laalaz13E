...
class TFProcess:
    ...
    def init_net(self, next_batch):
        self.x = next_batch[0]  # tf.placeholder(tf.float32, [None, 18, 13 * 13])
        self.y_ = next_batch[1] # tf.placeholder(tf.float32, [None, 13*13+1])
    ...
    def process(self, batch_size):
        ...
        if steps % 12000 == 0:
            print("begin to write new weight file(step 1)\n")
            sum_accuracy = 0
            sum_mse = 0
            sum_policy = 0
            test_batches = 100
            for _ in range(0, test_batches):
                print("begin to run session: {}".format(_))
                ...
                print("session {} completed".format(_));
            print("begin to write new weight file(step 2)\n")
            ...
            print("begin writing weight file \n")
    ...
    def construct_net(self, planes):
        ...
        x_planes = tf.reshape(planes, [-1, 18, 13, 13])
        ...
        conv_pol = self.conv_block(flow, filter_size=1,
                                   ...
                                   output_channels=2)
        h_conv_pol_flat = tf.reshape(conv_pol, [-1, 2*13*13])
        W_fc1 = weight_variable([2 * 13 * 13, (13 * 13) + 1])
        b_fc1 = bias_variable([(13 * 13) + 1])
        ...
        conv_val = self.conv_block(flow, filter_size=1,
                                   input_channels=self.RESIDUAL_FILTERS,
                                   output_channels=1)
        h_conv_val_flat = tf.reshape(conv_val, [-1, 13*13])
        W_fc2 = weight_variable([13 * 13, 256])
